...
export default function LoginForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const defaultPath = sanitizeSessionLandingPath(payload.landing_path) ?? resolveLandingRoute(payload.role);
    const from = new URLSearchParams(window.location.search).get("from");
    router.push(safeRedirectTarget(from, defaultPath));
  }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#F7F6F3] px-4 py-16">
      <div className="w-full max-w-[380px]">
        {/* Wordmark */}
        <div className="mb-8 flex justify-center">
          <div className="flex items-center gap-2.5">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[var(--color-primary-600)]">
              <svg viewBox="0 0 16 16" fill="currentColor" className="h-3.5 w-3.5 text-white" aria-hidden="true">
                <rect x="1" y="5" width="2.5" height="6" rx="1.25"/>
                <rect x="4.75" y="3" width="2.5" height="10" rx="1.25"/>
                <rect x="8.5" y="1" width="2.5" height="14" rx="1.25"/>
                <rect x="12.25" y="3.5" width="2.5" height="9" rx="1.25"/>
              </svg>
            </div>
        ...
        <div className="rounded-xl border border-[var(--color-edge)] bg-white px-7 py-7 shadow-sm">
          <h1 className="text-[20px] font-semibold tracking-tight text-[var(--color-text)]">Welcome back</h1>
          ...
          {devTokenAuthEnabled ? (
            ...
            <details className="group mt-6">
              ...
              <form onSubmit={handleSubmit} className="mt-3 space-y-2">
                <label htmlFor="access_token" className="block text-[11px] font-medium text-[var(--color-text-secondary)]">
                  {tokenLabel}
                </label>
